...
**Critères d'acceptation :**
- Le frontend ne fait aucune écriture sur le stockage partagé (lecture seule stricte, conforme à `architecture.md`). **MàJ V3 (2026-09-10) :** ce critère est remplacé par une frontière plus précise — le frontend écrit `propositions`, `commentaires` et `comptes`, jamais `articles`, `scores` ni `mise_en_contexte`. Aucun clic ne peut fabriquer, modifier ou effacer un verdict, ce qui était la garantie réellement visée. Cf. `architecture.md`, décision V3, et `doc/V3/`.
...
- Toute action d'écriture (validation manuelle, correction de score par un humain) n'est pas couverte — le frontend reste strictement en lecture seule à ce stade. **MàJ V3 (2026-09-10) :** la participation humaine est désormais scopée (`doc/V3/`) — proposition d'articles, file d'attente décidée par les contributeurs, commentaires. La **correction d'un score par un humain** reste, elle, hors périmètre : les commentaires n'influencent pas le score composite (cf. `doc/V3/userstories_crowdsourcing.md`, US-08, non-objectif explicite).
