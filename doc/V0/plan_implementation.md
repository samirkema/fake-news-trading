...
- **Phase 0** : terminée.
- **Phase 1 (scraper)** : US-01 (RSS — flux fiables/satiriques bilingues en place ; flux douteux anglophones actifs, domaines douteux francophones référencés dans `reputation.DOUTEUX`, mais flux RSS douteux FR actifs non collectés par manque de flux stables répondant sans mur anti-bot — décision tracée, cf. audit phase 12), US-02 (Reddit), US-04 (normalisation), US-05 (déduplication), US-06 (GitHub Actions) terminées. US-03 (GDELT) non implémentée, décision assumée (cf. `doc/userstories_scraper.md` US-03 et les audits de suivi) — l'évaluateur a un repli par similarité prévu pour ce cas.
...
- **Phase 5 (automatisation)** : orchestration scraper → évaluateur → contextualiseur en place (`pipeline_hebdomadaire.yml`), génération de mise en contexte réellement branchée. ⚠️ Le `schedule` hebdomadaire est **commenté** (projet en pause, pour ne pas consommer la clé Anthropic) : le workflow ne part que sur déclenchement manuel.
- **Phase 6 (correctifs d'audit)** : audit complet de la codebase, 33 findings corrigés — CI avec vraie base et budget de skips à zéro, authentification fail-closed, session expirable, plafond sur `/login`, contrainte `superadmin` avec code personnel, encadrement du contenu tiers dans les prompts LLM, US-04 source primaire remise en conformité (requête sur termes distinctifs, pénalité conditionnée à une claim vérifiable, correspondance de ticker sur jetons entiers), traçabilité US-08 persistée (`scores.detail_calcul`).
- **Phases 7 à 13 (relectures d'audit et consolidations)** : passages successifs sur les correctifs eux-mêmes. Acquis vérifiés par mutation : les cinq plafonds et le débit SEC tuent chacun un test dédié ; injections de prompt neutralisées dans les justifications de signaux ; négations françaises du fact-checking et détection stylistique bilingues ; intégrité HTML à la collecte ; en-têtes HTTP de sécurité ; accessibilité du formulaire de connexion. La phase 13 a par ailleurs annulé un plafond anti-bruteforce posé sur la garde d'accès : partagé derrière le proxy, il fermait le site à des porteurs de cookie valide. Rapports dans `doc/audit/`.
...
**Note pour la prochaine mise à jour de cette section** : elle s'est déjà désynchronisée une fois, dans le tour même qui a livré la Phase 4 (cf. audit de suivi `doc/audit/audit-phase4-frontend.md`) — la mettre à jour fait partie de "finir" une phase, pas une étape séparée qu'on peut oublier.
