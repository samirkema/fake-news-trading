...
- **Phases 7 à 9 (relectures d'audit)** : trois passes successives sur les correctifs eux-mêmes, 15 puis 9 puis 10 findings, tous traités — plafond de connexion rendu non contournable et sans déni de service, `non_evaluable` réellement atteignable, et garde-fou de schéma au démarrage (les migrations passent avant le code, cf. `fakenews.schema`). Rapports dans `doc/audit/`.
